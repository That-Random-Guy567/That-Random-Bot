...
class Client(commands.Bot):
    ...
    async def on_ready(self):
        ...
        self.ping_interval = 60  # 1 minute in seconds           # time for bump reminders
        self.normal_message_interval = 10  # 10 seconds
    ...
    async def bump_reminder_loop(self, channel):  # Separate loop function
        while True:
            ...
            await asyncio.sleep(120)  # Refreshing script to see when to execute command. 
    ...
    async def bump_reminder(self, channel):
        ...
        if current_time - self.last_ping_time >= self.ping_interval:
            if role:
                await channel.send(f"Time to bump {role.mention}")  # do @bump
                print("Bump reminder (with ping) sent.")
            else:
                print("Bump role not found for ping.")
            self.last_ping_time = current_time

        if current_time - self.last_normal_message_time >= self.normal_message_interval:
